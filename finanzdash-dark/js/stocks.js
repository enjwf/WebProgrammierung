...
import { AV, $, fmtUSD, fmtPct, debounce, spinner, loadWatchlist, saveWatchlist, getDailyData, FMP } from './utils.js';
...
function renderWL() {
  ...
  list.forEach(sym => {
    ...
    row.innerHTML = `<div><strong>${sym}</strong></div><div class="d-flex gap-2">
      <a class="btn btn-sm btn-outline-light open-link" href="#${sym}">Öffnen</a>
      <button class="btn btn-sm btn-outline-danger remove-btn">Entfernen</button>
    </div>`;
    ...
    if (removeBtn) removeBtn.addEventListener('click', () => wlRemove(sym));
